mira, observa como esta mi singleton @userReferences.service.ts , rechazé el uso de cache porque yo desde el frontend personalmente  me ocupo de ello con hooks como react-query y zustand, quiero que revises meticulosamente si este servicio esta correctamente construido, como ya sabras se trata del backend por lo que preciso los mejores mecanismos, creo  que asi  como esta el servicio de userReferences podria decir algo como esto y por favor corrigeme si he dicho algo incorrecto; cuando llamamos al middleware desde la ruta @location.routes.ts  lo que hace es  que antes de  @loadUserReferences.middleware.ts  obtenermos las cookies con el token id del  usuario, luego con este mismo dato pasamos a load UserReferences, aqui validamos que exista el id usuario para trabajar, segido llamamos a userReference.service @userReferences.service.ts quisiera que miraras el constructor, quizas no esté del todo optimo, quizas le haga falta algo, entonces retorna el findById, lo que no se es en que momento se  usa el getCity o los otros metodos de esta clase, no lo se, por favor despeja mis inquietudes, me estoy confundiendo un poco, puedes acceder y mirar  todo mi repertorio de carpetas y archivos @server @src @middlewares @services @controllers @routes @libs @utils @models @interfaces animo, dame el analisis mas profesional y las mejores implementaciones, quiero impresionar a mi CEO
